...
from monai.networks.nets.basic_unet import BasicUNet, Down, TwoConv, UpCat
...
class BasicUNetNoSkip(nn.Module):
    def __init__(
        self,
        spatial_dims: int = 3,
        in_channels: int = 1,
...
class BasicUNetZeroSkip(BasicUNet):
    def __init__(
        self,
        spatial_dims: int = 3,
        in_channels: int = 1,
        ...
        with redirect_stdout(f):
            super().__init__(spatial_dims, in_channels, out_channels, features, act, norm, bias, dropout, upsample)
        fea = ensure_tuple_rep(features, 6)
        print(f"BasicUNetZeroSkip features: {fea}.")

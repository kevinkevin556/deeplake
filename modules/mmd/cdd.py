...
class CDDModule(MMDModule):
    ...
    def forward(self, x):
        ...
        output = self.predictor((skip_outputs, self.bn(feature)))
        return output
...
class CDDUpdater(BaseUpdater):
    ...
    def update(self, module, images, masks, modalities):
        ...
        ct_output = module.predictor((no_skip_outputs, feat_repr[:batch_size]))
        mr_output = module.predictor((no_skip_outputs, feat_repr[batch_size:]))
        ...
        ct_class_repr = torch.cat(
            [module.feat_extractor(ct_image * (ct_plabel == c + 1))[1] for c in range(num_classes)]
        )
        ct_class_repr = ct_class_repr.reshape(batch_size * num_classes, feature_dim)

        mr_class_repr = torch.cat(
            [module.feat_extractor(mr_image * (mr_plabel == c + 1))[1] for c in range(num_classes)]
        )
